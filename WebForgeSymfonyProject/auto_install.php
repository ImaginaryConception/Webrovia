...
// === CONFIG === //
$baseDir = __DIR__;
$maxProjects = 100; // Limite de 100 dossiers à scanner

// === Contenu des fichiers à injecter === //
$composerJson = <<<'JSON'
{
    "type": "project",
    "license": "proprietary",
    ...
    "require": {
        ...
        "twig/twig": "^2.12|^3.0",
        "victor-prdh/recaptcha-bundle": "^1.0"
    },
    "config": {
        "allow-plugins": {
...
$composerLock = <<<'JSON'
{
    ...
    "content-hash": "a12f85db1ec55e615b69bb30aa2c6f36",
    "packages": [
        {
        ...
        },
        {
            ...
            "source": {
                ...
                "reference": "c2c3c71ee9cadc33424f8ab4076bec46fad96606"
            },
            "dist": {
                "type": "zip",
                "url": "https://api.github.com/repos/victor-prdh/recaptcha-bundle/zipball/c2c3c71ee9cadc33424f8ab4076bec46fad96606",
                "reference": "c2c3c71ee9cadc33424f8ab4076bec46fad96606",
                "shasum": ""
            },
            "require": {
            ...
            "support": {
                ...
                "source": "https://github.com/victor-prdh/recaptcha-bundle/tree/1.9.1"
            },
            "time": "2024-12-31T16:22:52+00:00"
        }
    ],
    "packages-dev": [
        {
...
// === Contenu du fichier bin/console === //
$consoleScript = <<<PHP
#!/usr/bin/env php
<?php
...
for ($i = 1; $i <= $maxProjects; $i++) {
    ...
    if (is_dir($projectDir)) {
        $vendorExists = is_dir($projectDir . "/vendor");
        $composerExists = file_exists($projectDir . "/composer.json");

        if (!$vendorExists && !$composerExists) {
            // Écrire les fichiers de config
            file_put_contents($projectDir . "/composer.json", $composerJson);
            file_put_contents($projectDir . "/composer.lock", $composerLock);
            file_put_contents($projectDir . "/symfony.lock", $symfonyLock);
            ...
            $consolePath = $binDir . "/console";
            file_put_contents($consolePath, $consoleScript);
            chmod($consolePath, 0755); // Exécutable

            // Composer install
            $phpPath = '/usr/local/bin/php';
            $composerPath = '/opt/cpanel/composer/bin/composer';

            $command = escapeshellcmd("$phpPath $composerPath install --no-interaction --prefer-dist");
            exec("cd " . escapeshellarg($projectDir) . " && $command 2>&1", $output, $returnVar);

            // Log
            $log = "[" . date('Y-m-d H:i:s') . "] Composer install dans dossier $i terminé (code: $returnVar)\n";
            $log .= "Sortie :\n" . implode("\n", $output) . "\n\n";
            file_put_contents($baseDir . "/install_log.txt", $log, FILE_APPEND);
